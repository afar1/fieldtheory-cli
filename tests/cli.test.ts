...
test('ft possible mark command is registered', () => {
  const program = buildCli();
  const possibleCmd = program.commands.find((c: any) => c.name() === 'possible');
  assert.ok(possibleCmd, 'possible command should be registered');
  const markCmd = possibleCmd.commands.find((c: any) => c.name() === 'mark');
  assert.ok(markCmd, 'possible mark command should be registered');
  assert.ok(markCmd.options.some((o: any) => o.long === '--note'));
});
