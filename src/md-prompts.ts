...
export interface ReconcileEntry {
  /** Wiki-relative path to the entry, no `.md` extension — e.g. `entries/2026-04-15-foo`. */
  relPath: string;
  /** Full file content of the entry, frontmatter + body. */
  content: string;
}

/**
 * Builds the prompt for integrating authored entries into an existing compiled page.
 *
 * Called by `reconcileEntries()` after `ft wiki` has regenerated bookmark-driven pages.
 * The prompt instructs the LLM to weave entry insights into the existing page
 * without removing any bookmark citations or inventing claims. This is the
 * "LLM-maintained wiki" operation described in Karpathy's LLM Wiki gist
 * (https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f):
 * "When you add a new source, the LLM ... integrates it into the existing wiki
 * — updating entity pages, revising topic summaries, noting where new data
...
export function buildReconcilePrompt(
  ...
  const entriesSection = entries.map((e, i) =>
    `### Entry ${i + 1}: ${e.relPath}\n\n\`\`\`markdown\n${e.content}\n\`\`\``
  ).join('\n\n');

  return `You are integrating authored knowledge entries into an existing wiki page.

${SECURITY_NOTE}

## Your task

Update the wiki page at "${targetPath}" to integrate relevant insights from ${entries.length} authored entries. The existing page was compiled from bookmarks and has established citations. Your job is to weave new information from the entries into the page WITHOUT:
- Removing any existing content or bookmark citations
- Breaking the YAML frontmatter
- Inventing facts beyond what is in the entries or existing page
- Changing \`source_count\` or \`source_type\` in the frontmatter

## Rules

1. Preserve every existing bookmark citation of the form [(source)](https://...).
2. When an entry contradicts the existing page, add or extend a "## Contradictions" section noting the disagreement explicitly.
3. When an entry extends existing content, integrate it under the most relevant existing section. Add a new section only when no existing section fits.
4. Preserve existing [[wikilinks]] and add new ones from the entries where they fit naturally.
5. Update the \`last_updated\` field in the frontmatter to ${today}.
6. Do not add a "Generated by" footer or any preamble. Output ONLY the updated markdown page.
...
Now output the updated wiki page. Output ONLY the markdown — no preamble, no explanation.`;
}
